  ...
  save.addEventListener("click",
    function (e) {
      ...
      if (key == "" || value == "") {
        ...
        let w_confirm = confirm("Local Storageに" + key + " " + value + "を保存(save)しますか？");
        if (w_confirm === true) {
          localStorage.setItem(key, value);
          viewStorage();
          let w_msg = "Local Storageに" + key + " " + value + "を保存(save)しました。";
          window.alert(w_msg);
  ...
  del.addEventListener("click",
    function (e) {
      ...
      if (w_cnt >= 1) {
        //const key = document.getElementById("textKey").value;
        //const value = document.getElementById("textMemo").value;
        let w_confirm = window.confirm("Local Storageから選択されている" + w_cnt + "件を削除(delete)ますか？");
